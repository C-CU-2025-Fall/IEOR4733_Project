#!/usr/bin/env python3
"""Generate Table 3 comparison across Long, MACD, and Sign(R) strategies with markdown output."""
...
def generate_markdown_table(asset_name: str, all_results: dict[str, dict]) -> str:
    ...
    for strategy in ["Long", "Sign(R)", "MACD"]:
        ...
        values = [format_metric(metrics.get(col, np.nan)) for col in METRIC_NAMES]
        lines.append(f"| {strategy:<8} |" + " | ".join(f" {v:>7}" for v in values) + " |")
...
def main():
    ...
    markdown_content = "# Table 3: Experiment Results for the Raw Signal\n\n"
    markdown_content += "Three strategies: **Long**, **Sign(R)**, and **MACD**\n\n"
    
    for asset in ASSETS:
        markdown_content += f"\n## {asset}\n\n"
        markdown_content += generate_markdown_table(asset, all_strategy_results)
        markdown_content += "\n\n"
    
    markdown_content += "\n---\n\n"
    markdown_content += "### Notes:\n"
    markdown_content += "- **Long**: Experimental upper bound with source violations (41/45 score)\n"
