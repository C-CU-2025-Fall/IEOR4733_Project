...
from drl_shared.spec import RETRAIN_ROUNDS, feature_data_path, round_name, universe_tickers
from drl_shared.state_space import build_contract_arrays


def prepare_contract_round_features(ticker: str, round_num: int) -> bool:
    ticker = ticker.upper()
    ...
    print(
        f"  {ticker}: prepared features train={len(df_train)}d "
        f"({round_info['train_start']}~{round_info['train_end']}), source={source}"
    )
    return True


def prepare_round_features(asset_name: str, round_num: int) -> tuple[int, int]:
    ...
    for ticker in tickers:
        if prepare_contract_round_features(ticker, round_num):
            ok += 1
        else:
            fail += 1
