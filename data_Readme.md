...
## 🔬 交叉验证 v3 结果 (2026-04-12 20:50)

**脚本**: `tests/cross_validate_v3.py`

### 验证逻辑

1. **ASC 和 RAD 都是调整后的连续合约** (不同基准，收益率应一致)
2. **ASC 覆盖期**: 验证 ASC vs RAD 收益率相关性 → 验证 RAD 方法正确
...
| 等级 | 标准 | 合约数 | 比例 |
|------|------|--------|------|
| A | ASC 验证通过 + 测试期 corr≥0.95 | 17 | 34% |
| B | ASC 验证通过 或 测试期 corr≥0.90 | 23 | 46% |
| C | 测试期 corr<0.90 | 10 | 20% |
| F | 数据缺失 | 0 | 0% |

**✅ 40/50 (80%) 合约 A/B 级，可用于回测**

### A 级合约 (17 个)
EN, ER, ES, SC, MD, YM, NK, SP, ZA, ZP, CA, AN, CN, DX, LX, XU, XX

### B 级合约 (23 个)
SB, SN, ZC, ZG, ZI, ZK, ZL, ZW, CC, GI, KC, KW, DT, UB, BN, FN, JN, MP, US, ZH, ZN, ZU, ZF

### C 级合约 (10 个) ⚠️
NR, TY, ZF, ZO, ZR, ZT, ZZ, DA, JO, LB, FB

**C 级原因**: 测试期 RAD/NON 相关性低 (<0.90)，可能数据质量问题

### 关键结论

- **18/50 合约经 ASC 验证**，RAD 调整方法可靠
- **即使 ASC 不覆盖测试期**，已验证的方法可信任
- **80% 合约 A/B 级**，数据质量足够支持回测

---

**最后更新**: 2026-04-12 20:50 EDT
