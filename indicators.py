...
def compute_macd(prices: ArrayLike, short_span: int, long_span: int) -> np.ndarray:
    ...
    Args:
        prices: Price series
        short_span: Short-term time scale (paper: 8, 16, 32)
        long_span: Long-term time scale (paper: 24, 48, 96)
...
def compute_macd_multi_scale(prices: ArrayLike) -> np.ndarray:
    """
    Multi-scale MACD averaging (Paper Formula 12)
...
def normalize_return(returns: np.ndarray, vol: np.ndarray, horizon: int = 252) -> np.ndarray:
    """
    Paper: Adjust daily returns to a reasonable time scale using volatility
    ...
    Args:
        ...
        horizon: Time scale (21/42/63/252, etc.)
...
def normalize_prices(prices: np.ndarray) -> np.ndarray:
    """
    Normalize closing price series
...
class FeatureEngineer:
    ...
    def build_features(self, prices: np.ndarray, returns: np.ndarray, 
        ...
        """
        Build state features at time step current_idx
        ...
        vol = np.nan_to_num(vol, nan=0.01, posinf=1.0, neginf=0.001)  # ✅ Handle vol NaN
        vol_mean = np.mean(vol) + 1e-10
        
        # Feature 1: Normalized closing price
...
def test_indicators():
    ...
    print("Testing technical indicator computations...")
