...
def main():
    ...
    parser.add_argument('--asset', default='all', choices=['Forex', 'Commodity', 'Equity Index', 'Fixed Income', 'all'])
    ...
    for asset_name in asset_classes:
        tickers = ASSET_CLASSES[asset_name]
        print(f"\n{'='*70}")
        print(f"Asset Class: {asset_name} ({len(tickers)} contracts)")
        print(f"{'='*70}")
        
        rounds_to_run = [args.round] if args.round else [1, 2]
        ...
        for round_num in rounds_to_run:
            ...
            for tk in tickers:
                rname = f'r{round_num}'
                if round_num == 1:
                    prepare_round(tk, rname, '2005-01-01', '2009-12-31', '2010-01-01', '2014-12-31')
                else:
                    prepare_round(tk, rname, '2005-01-01', '2014-12-31', '2015-01-01', '2019-12-31')
