...
class ContractEnv:
    ...
    def step(self, action_id):
        ...
        sig_prev = self.sigma[self.idx - 1]
        vol_scale = self.sigma_tgt / sig_prev if sig_prev > 0 else 0.0

        gross = action * vol_scale * self.returns[self.idx]
        tc = BP * self.prices[self.idx - 1] * abs(action * vol_scale - self.last_action * vol_scale)
        reward = gross - tc
...
def train_contract(ticker, source='RAD', episodes=EPISODES, verbose=True):
    """Train DQN for a single contract with checkpointing."""
    t0 = time.time()
    ...
    if verbose:
        print(f"  {ticker}: ✅ {elapsed:.1f}s, final_avg_r={final_avg:+.4f}")
...
def train_asset_class(asset_name, episodes=EPISODES):
    ...
    for i, tk in enumerate(tickers):
        print(f"\n[{i+1}/{len(tickers)}] {tk}:")
        if train_contract(tk, episodes=episodes):
            ok += 1
        else:
            fail += 1
...
if __name__ == '__main__':
    ...
    if a.cmd == 'status':
        ...
        if a.ticker:
            train_contract(a.ticker, source=a.source, episodes=a.episodes)
        elif a.asset:
            train_asset_class(a.asset, episodes=a.episodes)
        else:
