...
# ============================================================
# 5b) New reward: covariance-penalized rollout
#     R_corr = R_orig - lambda_corr * w.T @ Sigma_t @ w
# ============================================================
def build_cov_lookup_newreward(state_dict, tickers, lookback=60, ret_col="ret_1d"):
    """
    Build a date -> covariance matrix lookup using only past returns.
    Sigma_t is estimated from [t-lookback, ..., t-1], so it does not use future data.
    """
    frames = []
    ...
    for ticker in tickers:
        if ticker not in state_dict:
            continue
        df = state_dict[ticker]["aligned_df"][["date", ret_col]].copy()
        df["date"] = pd.to_datetime(df["date"])
        df = df.rename(columns={ret_col: ticker})
        frames.append(df)
    ...
    cov_lookup = {}
    for i in range(lookback, len(wide)):
        date = pd.to_datetime(wide.index[i])
        hist = wide.iloc[i - lookback:i]   # past data only
        cov_lookup[date] = hist.cov()

    return cov_lookup


def collect_rollout_newreward(
    envs,
    actor,
    critic,
    rollout_steps,
    ...
    if not hasattr(collect_rollout_newreward, "_current_states") or len(collect_rollout_newreward._current_states) != len(envs):
        collect_rollout_newreward._current_states = [env.reset() for env in envs]

    current_states = collect_rollout_newreward._current_states
    ...
    for _ in range(rollout_steps):
        ...
        # The only reward-function change:
        # R_corr = R_orig - lambda_corr * w.T @ Sigma_t @ w
        if cov_lookup is not None and lambda_corr > 0:
            dates = [pd.to_datetime(info.get("date_t")) for info in info_batch]

            # The covariance penalty is a portfolio-level term, so apply it only when
            # all parallel environments correspond to the same date.
            if len(set(dates)) == 1:
                date_t = dates[0]

                if date_t in cov_lookup:
                    tickers = [info.get("ticker") for info in info_batch]
                    Sigma = (
                        cov_lookup[date_t]
                    ...
                    # w is the equal-weight portfolio vector of volatility-scaled positions.
                    w = np.array([info.get("scaled_pos_t", 0.0) for info in info_batch], dtype=float)
                    w = w / len(w)

                    penalty = lambda_corr * float(w.T @ Sigma @ w)
                    reward_batch = [r - penalty for r in reward_batch]
    ...
    collect_rollout_newreward._current_states = current_states
...
def fit_newreward(
    ...
    # avoid rollout state leaking across asset classes / separate runs
    if hasattr(collect_rollout_newreward, "_current_states"):
        del collect_rollout_newreward._current_states

    for update_idx in range(start_update + 1, start_update + n_updates + 1):
        batch = collect_rollout_newreward(
            envs=envs,
            actor=self.actor,
            critic=self.critic,
            rollout_steps=rollout_steps,
...
PaperA2CTrainer.fit_newreward = fit_newreward
...
# ============================================================
# 8b) Build envs from state_dict for new reward
#     Same as build_envs_from_state_dict, but passes ticker into the environment.
# ============================================================
def build_envs_from_state_dict_newreward(
    state_dict,
    tickers=None,
    sigma_target=None,
    bp=BP,
