#!/usr/bin/env python3
"""Run the legacy experimental configuration against both Table 3 and Table 2."""
        ...
        for asset_name in ["Commodity", "Equity Index", "Fixed Income", "Forex"]:
            raw.extend(
                load_contracts(
                    ...
                    test_start,
                    test_end,
                    excluded_contracts=fe.LEGACY_EXPERIMENTAL_EXCLUDED,
                    source_overrides=fe.LEGACY_EXPERIMENTAL_OVERRIDES,
                )
            )
        return raw
...
def print_legacy_header(args):
    print("Legacy experimental configuration")
    print("=" * 60)
...
def main():
    parser = argparse.ArgumentParser(
        description="Run legacy experimental configuration for Table 2 and/or Table 3"
    )
