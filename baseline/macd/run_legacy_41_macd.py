#!/usr/bin/env python3
"""Run the legacy experimental MACD configuration against both Table 3 and Table 2."""
        ...
        for asset_name in ["Commodity", "Equity Index", "Fixed Income", "Forex"]:
            raw.extend(
                load_contracts(
                    ...
                    test_start,
                    test_end,
                    excluded_contracts=fe.LEGACY_EXPERIMENTAL_EXCLUDED_MACD,
                    source_overrides=fe.LEGACY_EXPERIMENTAL_OVERRIDES_MACD,
                )
            )
        return raw
...
def main():
    parser = argparse.ArgumentParser(description="Run legacy MACD configuration for Table 2 and/or Table 3")
    ...
    print('Legacy experimental MACD configuration')
    print('=' * 60)
