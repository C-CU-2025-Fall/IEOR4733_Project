#!/usr/bin/env python3
"""
Enumerate clean same-rule and 40+ first Table 3 Sign(R) frontiers under one scorecard.
...
Goals:
  ...
  4. find Sign(R)-specific optimal data source overrides
"""
...
from config import ASSET_CLASSES, METRIC_NAMES, PAPER_TABLE3, SOURCE_OVERRIDES, LEGACY_EXPERIMENTAL_OVERRIDES_SIGNR, LEGACY_EXPERIMENTAL_EXCLUDED_SIGNR  # noqa: E402
...
def evaluate_scenario(
    ...
    for asset in ASSETS4:
        ...
        trade_returns = compute_portfolio_returns(raw, strategy, SIGMA, aggregation_mode="variable_n")  # ← Sign(R)
    ...
    all_trade_returns = compute_portfolio_returns(raw_all, strategy, SIGMA, aggregation_mode="variable_n")  # ← Sign(R)
    ...
    for numerator_mode, all_mode in itertools.product(NUMERATOR_MODES, ALL_MODES):
        rows.append(
            scenario(
                ...
                overrides=LEGACY_EXPERIMENTAL_OVERRIDES_SIGNR,
                excluded=LEGACY_EXPERIMENTAL_EXCLUDED_SIGNR,
...
def main():
    print("=" * 80)
    print(f"Searching for Sign(R)-specific optimal data source frontiers")
    print(f"Strategy: {STRATEGY}")
    print("=" * 80)
    
    legacy_rows = search_legacy_experimental()
    
    print("\nTop 10 Sign(R) Configurations (by 4-asset <=15 score):")
    print("-" * 80)
    ...
    best = legacy_rows[0]
    print("\n" + "=" * 80)
    print("BEST SIGN(R) CONFIGURATION")
    print("=" * 80)
