...
This note explains why I do **not** believe the paper *Deep Reinforcement Learning for Trading* can be fully and faithfully reproduced from the published text alone. The issue is not simply that the implementation is difficult; rather, the paper contains a combination of:

1. **hard internal inconsistencies**,  
2. **underspecified metric definitions**, and  
3. **evaluation choices that are not sufficiently pinned down for replication**.

In my view, this means that partial reproduction is possible, but **full table-level reproduction across all asset classes is not realistically achievable from the paper alone**. :contentReference[oaicite:0]{index=0}
...
> “the Calmar ratio compares the expected annual rate of return with maximum drawdown” :contentReference[oaicite:1]{index=1}

The natural reading of this statement is:

\[
\text{Calmar} = \frac{E(R)}{MDD}
\]
...
- \(E(R)=0.668\)
- \(MDD=0.132\)
- reported Calmar = **0.509** :contentReference[oaicite:2]{index=2}

But if Calmar were truly computed as \(E(R)/MDD\), then:

\[
0.668 / 0.132 \approx 5.06
\]
...
- \(E(R)=0.504\)
- \(MDD=0.127\)
- reported Calmar = **0.466** :contentReference[oaicite:3]{index=3}

But:

\[
0.504 / 0.127 \approx 3.97
\]
...
Therefore, **Calmar cannot be treated as a faithfully reproducible metric from the paper text**. :contentReference[oaicite:4]{index=4} :contentReference[oaicite:5]{index=5}
...
> “annualised standard deviation of trade returns that are negative” :contentReference[oaicite:6]{index=6}
...
\[
DD = \sqrt{252} \cdot std(R_t \mid R_t < 0)
\]

### Interpretation B: zero-target downside deviation / lower partial moment
\[
DD = \sqrt{252} \cdot \sqrt{\frac{1}{T}\sum_{t=1}^{T}\min(R_t,0)^2}
\]

These are **not the same quantity**.

What makes this especially problematic is that the paper’s **Sortino values do appear numerically consistent with the reported DD column**, which means Sortino is internally consistent **conditional on DD**. But DD itself is not defined rigorously enough to guarantee that a third party will compute the same thing. :contentReference[oaicite:7]{index=7}
...
> “We define \(r_t = p_t - p_{t-1}\) and this expression represents additive profits.” :contentReference[oaicite:8]{index=8}

It further contrasts this with multiplicative returns:

> “If we want to trade a fraction of our accumulated wealth at each time, multiplicative profits should be used and \(r_t = p_t/p_{t-1} - 1\).” :contentReference[oaicite:9]{index=9}
...
- maximum drawdown,
- Calmar ratio,
- cumulative trade returns, and
- portfolio-level volatility scaling. :contentReference[oaicite:10]{index=10}
...
- \(W_t = W_0 + \sum_{s \le t} R_s\),
- \(W_t = N W_0 + \sum_{s \le t} R_s\),
- a normalized NAV,
...
That leaves too much freedom in reproducing MDD-related metrics. :contentReference[oaicite:11]{index=11} :contentReference[oaicite:12]{index=12}

---

## 4. The paper mixes multiple layers of volatility scaling without fully operationalizing them

In the reward definition, the paper says that positions are volatility scaled by \(\sigma_{tgt}/\sigma_t\), and that this helps normalize rewards across contracts with different price scales. :contentReference[oaicite:13]{index=13}
...
> “We also include the results without this volatility scaling for reference in Table 3.” :contentReference[oaicite:14]{index=14}

So there are at least **two distinct layers of scaling**:

1. **contract-level scaling inside the reward function**, and  
...
The paper clearly states that **an extra portfolio-level scaling layer exists**, but it does not specify it in enough detail to make third-party reproduction unique. :contentReference[oaicite:15]{index=15}
...
\[
R_t^{port} = \frac{1}{N}\sum_{i=1}^{N} R_t^i
\]
...
- equity indices,
- fixed income,
- and FX, :contentReference[oaicite:17]{index=17}
...
- scaling,
- cost treatment,
- and evaluation path construction
...
- Sharpe as \(E(R)/std(R)\),
- the existence of contract-level and portfolio-level volatility scaling. :contentReference[oaicite:18]{index=18} :contentReference[oaicite:19]{index=19}
...
- **Calmar ratio as reported in the tables**, because the numerical values contradict the written definition. :contentReference[oaicite:20]{index=20} :contentReference[oaicite:21]{index=21}
...
- the reported **Calmar values do not match the paper’s own written formula**. :contentReference[oaicite:22]{index=22} :contentReference[oaicite:23]{index=23}
...
- the exact implementation of portfolio-level volatility targeting. :contentReference[oaicite:24]{index=24} :contentReference[oaicite:25]{index=25}
